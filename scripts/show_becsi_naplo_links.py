...
if 'class="naplo-links-visible"' not in s:
    pat=re.compile(r'(<div class="hierarchy-card[^"]*"[^>]*>.*?<h3>Bécsi Napló</h3>)(.*?)(</div>)', re.S)
    m=pat.search(s)
    if not m:
        raise SystemExit('Bécsi Napló hierarchy card not found')
...
<a href="https://www.becsinaplo.eu/" target="_blank" rel="noopener noreferrer"><strong>Archív anyagok</strong><span>www.becsinaplo.eu</span></a>
    ...
    replacement=m.group(1)+m.group(2)+links+m.group(3)
